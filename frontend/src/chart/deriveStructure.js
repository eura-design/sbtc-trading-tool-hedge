...
//
// [1] CHoCH는 "직전에 확정된 돌파로 bias가 선 상태"에서만 찍는다.
//     자동 ZZ처럼 느슨하게(첫 돌파부터 CHoCH) 바꾸지 말 것 — 아래 [2] 참고.
...
//
// ── 자동 ZZ와 판정이 다른 부분 ────────────────────────────────────────────────
// 자동 ZZ는 structHigh/structLow가 아직 없을 때(NaN) bias를 강제로 세팅한다
// (structureZigzag.js:143, 187). 그래서 H1 → L1 → H2(>H1) 같은 단순 상승에서도
// 첫 CHoCH가 찍힌다. 실제로는 하락 추세가 성립한 적이 없으므로 BOS이지 CHoCH가 아니다.
// 꼭짓점을 손으로 몇 개만 찍는 수동 구조에서는 이 오탐이 바로 눈에 띄므로,
// 여기서는 **실제 돌파가 있었을 때만 bias를 세운다.**
//
// ── 판정은 전부 꼬리(고가/저가) 기준 ──────────────────────────────────────────
