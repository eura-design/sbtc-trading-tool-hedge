...
// 개별 조절은 단축키 `[` `]` (0.1 단위) 또는 더블클릭 팝업.
// ⚠ 신규 구조 기본 투명도 — 2026-08-25 사용자 요청으로 **0.25 → 0.3**
//   (0.5 → 0.25는 2026-08-24).
//   자동 ZZ(indicatorParams.zz.opacity)와 **같은 값으로 맞춘 것**이다:
//   두 지표는 팝업 구성이 같은데 초기 수치만 달라 서로 다른 것처럼 보였다.
//   한쪽만 바꾸지 말 것 — 바꾸려면 자동 ZZ 기본값도 같이.
//   ※ 0.3으로 올린 이유는 **슬라이더 눈금에 맞추기 위해서**다. 0.25는 그전
//     슬라이더의 최소값이었는데, 같은 날 슬라이더가 10% 단위·하한 0.1이 되면서
//     눈금 위의 값이 아니게 됐다 (끌면 20이나 30으로 붙어버린다)
//   ※ 이미 그려 둔 구조는 자기 값을 들고 있어 그대로다 — 새로 그리는 것부터 적용된다
export const STRUCT_DEFAULT_OPACITY = 0.3;
