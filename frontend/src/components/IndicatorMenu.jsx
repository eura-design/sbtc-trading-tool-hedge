...
export const INDICATORS = [
  ...
  // ⚠ **피보나치 행을 다시 만들지 말 것** (2026-08-15 사용자 요청으로 제거).
  //   선/채널/원과 완전히 같은 도형인데 저것들만 지표 체크박스라는 관문을 갖고 있었고,
  //   "지표를 선택해야만 TopBar 버튼이 활성화되는 게 이상하다"가 없앤 이유다.
  //   여기 있던 유일한 근거(전역 레벨 목록)도 도형별로 옮겨 사라졌다 —
  //   레벨은 이제 도형 더블클릭 팝업에서 고른다 (chart/fib.js [F1])
  { key: "ema", label: "EMA" },
];
...
// ※ FibLevelPanel(피보나치 레벨 전역 선택)은 2026-08-15 제거됐다.
//   레벨이 도형별이 되면서 조작 위치가 더블클릭 팝업(LineOpacityPopup의 FibLevelRow)으로
//   옮겨졌다. 여기 다시 만들지 말 것 — 전역·도형별 두 값이 생기는 순간
//   어느 쪽이 이기는지가 화면 어디에도 안 드러난다 (chart/fib.js [F1])
