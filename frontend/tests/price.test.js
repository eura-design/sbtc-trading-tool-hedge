...
// ── R:R (2026-09-02) ────────────────────────────────────────────────────────
// ⚠ 하한이 0.1 고정이라 **저가 코인에서 R:R이 항상 `—`**로 떴다.
//   DOGE는 진입·손절 차이가 0.005 같은 값이라 0.1을 넘을 일이 없다.

import { calcRR } from "../src/utils/format.js";

test("저가 코인에서도 R:R이 계산된다 (회귀)", () => {
  // DOGE 롱: 진입 0.20 / TP 0.21 / SL 0.195 → 1:2
  assert.equal(calcRR(0.20, 0.21, 0.195, true, 0.00001), "2.00");
  // 옛 기준(0.1)이면 `—`였다
  assert.equal(calcRR(0.20, 0.21, 0.195, true, 0.1), "—");
});

test("BTC는 예전 그대로", () => {
  assert.equal(calcRR(72000, 75000, 70000, true, 0.1), "1.50");
  assert.equal(calcRR(72000, 75000, 71999.95, true, 0.1), "—");   // 한 칸 미만
});

test("손절이 이익 구간이면 무한대", () => {
  assert.equal(calcRR(0.20, 0.21, 0.205, true, 0.00001), "∞");
});
