...
// BTCUSDT의 실제 필터값 — **거래소 표를 못 받았을 때만** 쓰는 대비책이다.
//
// ⚠ 함부로 바꾸지 말 것: 바꾸면 "캐시가 비었을 때"의 동작이 조용히 달라진다.
//   바꿀 때는 **실제로 재보고, 왜 바꿨는지 여기 적는다.**
// ⚠ `minNotional`은 거래소가 바꾸는 값이다. 2026-09-06에 재보니 **50**인데 여기엔
//   `"100"`이 적혀 있었다(그전 값). 그동안 표를 못 받은 구간에서 작은 주문이
//   **필요보다 두 배 큰 수량**으로 올라갔다 — `calc.js`가 `minNotional`을 채우려고
//   수량을 올리기 때문이다. 사용자 확인 뒤 50으로 맞췄다.
//   ※ 언젠가 또 낡는다. 원본은 늘 `exchangeInfo`다 — 이 값에 기대는 구간을 늘리지 말 것
const SEED = {
  BTCUSDT: { symbol: "BTCUSDT", tickSize: "0.10", stepSize: "0.001",
             minQty: "0.001", minNotional: "50",          // 2026-09-06 실측
