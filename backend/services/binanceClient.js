...
async function cancelExistingAlgoTPSL(positionSide, symbol = SYM()) {
  try {
    ...
    //   맞는 동작이었다. 부분 손절(예: "평단까지 내려오면 절반")이 생기면 **그것까지 지워진다** —
    //   알림도 로그도 없이. 사용자는 절반 청산이 예약돼 있다고 믿는데 실제로는 없는 상태가 된다.
    //
    //   ⚠ **언제 지워지는가** (2026-09-04에 확인해 다시 적었다. 예전 주석은 "추가 진입 한 번이면
    //     사라진다"였는데 **그렇지 않다** — 추가 진입 체결은 store 기록만 지우고 끝이라
    //     `placeTPSL`을 부르지 않는다. 전량 TP/SL은 `closePosition`이라 수량이 없고,
    //     그래서 포지션이 커져도 다시 걸 이유가 없다):
    //       · 진입 주문이 체결될 때 (`onFilled` / 시장가 진입)
    //       · TP/SL이 없는 것을 발견해 **다시 걸 때** — reconcile의 retryable, 재시작 복구
    //     앞의 것은 같은 사이드에 포지션이 있으면 플랜 박스가 막혀서 흔치 않다.
    //     실제로는 **뒤의 둘**(무언가 잘못돼 수습하는 경로)에서 부분 손절과 마주친다.
    //
