...
- ⚠ **`INCOME`이 수익 곡선·승률·수수료·펀딩비의 유일한 출처다** (`services/incomeLogger.js`,
  ...
  - ⚠ 집계는 `incomeTime`(거래소가 찍은 시각)으로 — `ts`는 우리가 **기록한** 시각이라
    백필하면 몇 달 차이가 난다
  - 커서는 `logs/.income_cursor.json`. 메모리에만 두면 재시작마다 7일치가 다시 쌓인다
...
- 진행 중 레그도 같은 규칙 — `Structures.jsx`가 `pts[n-3]→pts[n-2]`를 `prev`로 실어 보낸다
  (`[R8]`, 꼭짓점 3개 미만이면 `null`). **`[R8]` 자체는 유지** — 예전엔 `prev`를 안 넘겨
  진행 중 레그만 비교가 안 됐다
...
- ※ `zz`의 **show_choch·max_choch는 두 곳에 있다** (2026-08-14 사용자 요청):
  ...
  - `zz.show_legvol`·`alert_choch`·`opacity`는 **팝업에만** 있다 (도형 공통 UI라 메뉴에 자리가 없다)
    ...
    - `opacity` 기본값은 **0.3**이다 (2026-08-25 — 그전엔 0.25) — 수동 구조 신규 기본값과 같은 값.
      ⚠ 이건 **서버에 저장되는 값**이라 기본값만 바꾸면 이미 저장된 쪽이 이긴다 —
        `backend/indicator_params.json`도 같이 봐야 하고, 바꾼 뒤 **백엔드 재시작**이 필요하다
