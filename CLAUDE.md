...
- `frontend/public/`
- `frontend/dist/`
- `ScreenShot/` (자동 저장된 진입/청산 스크린샷 PNG)
...
│   ├── position.js            ← GET /api/position
│   ├── order.js               ← POST /api/order (진입주문 + TP/SL 자동등록, 일일 손실 가드) / PATCH /api/order (미체결 TP/SL 수정)
│   ├── orders.js              ← DELETE /api/orders (전체 취소)
│   ├── close.js               ← POST /api/close (전량: TP/SL+SCALE_IN 취소 후 시장가 / 부분: 분할TP 비율 재등록)
│   ├── swap.js                ← POST /api/swap (포지션 방향 스왑 — 취소→청산→반대진입→TP/SL ±2% 자동등록)
│   ├── tpsl.js                ← GET/PUT /api/tpsl + POST/DELETE /api/tpsl/split (분할 TP)
│   ├── scalein.js             ← GET/POST/DELETE /api/scale-in (추가 진입)
│   ├── leverage.js            ← POST /api/leverage (레버리지 즉시 변경, 1~125)
│   ├── stats.js               ← GET /api/stats?startTime= (거래 통계: 승률, PnL, 수수료, 펀딩비) + statsCache 사용
│   ├── dailyloss.js           ← GET /api/daily-loss (총자본 5% 한도, UTC 0시 리셋) + checkDailyLoss() export
│   ├── indicatorparams.js     ← GET/POST /api/indicator-params (보조지표 파라미터 영속화)
│   ├── screenshot.js          ← POST /api/screenshot (base64 PNG → ScreenShot/YYYY-MM-DD_HHMMSS_SIDE_PRICE.png)
...
│   ├── orderWatcher.js        ← Binance User Data Stream (WebSocket 체결 감지) + reconcileWithBinance(15초)
...
│                                 API_BASE, BN_PUBLIC, INTERVALS, RSI_H, RSI_GAP, POLLING(폴링 주기 상수),
│                                 CANVAS_C(캔버스 색상 토큰: BULL/BEAR/AXIS/XTICK/YTICK)
...
│   ├── uiSlice.js             ← UI/드로잉/드래그 상태: drawing/drawMode/confirmInfo/criticalAlert/selectedBox/dragTpsl/dragScaleIn/dragSplitTp 등
│   └── orderSlice.js          ← 주문 액션: executeOrder/saveTpsl/scaleIn/addSplitTp/swapPosition/closePosition 등
├── utils/
│   ├── calc.js                ← calcPosition() — 리스크 기반 수량 계산
│   ├── format.js              ← calcRR() 등 포맷 유틸
│   ├── hitTest.js             ← distToSeg(), findHitLine() — 선분 히트 테스트
│   ├── captureChart.js        ← 캔들/볼륨/RSI 캔버스 합성 + 진입/청산 마커 → base64 PNG
...
│   ├── usePosition.js         ← 포지션 + pending 주문 + scaleInOrders (5초 폴링)
│   ├── useTpsl.js             ← TP/SL + splitTps 조회 (폴링, hasPos일 때만)
...
│   ├── usePositionCloseAlert.js ← 포지션 종료 감지 → sticky 알림 (롱/숏 포지션 종료 구분)
...
│   ├── useDrawing.js          ← 드로잉 박스 + 그리기 중 사각형 상태
│   ├── useScreenshot.js       ← 포지션 open/close 전이 감지 → captureChart → POST /api/screenshot
...
│   ├── cursorRules.js         ← CURSOR_RULES 배열(12규칙), getCursor(ctx) — 커서 결정 로직
...
│                                 채널: channel_ep/channel_move/channel_offset
...
│   ├── IndicatorMenu.jsx      ← 보조지표 온/오프 + 파라미터 설정 (Volume/RSI/RSI Divergence/S·R/OB/FVG/EMA)
...
│   ├── Divider.jsx            ← 구분선
│   ├── ConfirmModal.jsx       ← 주문 확인 팝업 (LIMIT/MARKET 선택)
...
│   │   ├── PositionLines.jsx      ← 체결 포지션 진입/TP/SL/분할TP/추가진입 수평선 (드래그 핸들)
...
│       ├── PositionCard.jsx   ← 포지션 정보 카드 + PendingCard + Swap 버튼(포지션 방향 반전)
│       ├── PlanCard.jsx       ← 드로잉 플랜 카드 + OrphanPendingCard
│       ├── ScaleInCard.jsx    ← 추가 진입 카드 (LIMIT/MARKET, 가격 방향 검증)
│       ├── SplitTPCard.jsx    ← 분할 TP 카드 (지정가, 잔여 수량 표시)
...
- `ScreenShot/` — 포지션 진입/청산 자동 스크린샷 PNG 저장 폴더 (useScreenshot 훅이 생성)
...
- **serverSlice**: balance, position, tpsl, liveClose + 폴링 훅용 refetch 콜백
- **settingsSlice**: riskPct, leverage, interval_, indicators (localStorage 동기화 + 값 변경 시 pending 주문 재등록 debounce)
- **uiSlice**: drawing, drawMode, orderStatus, confirmInfo, criticalAlert, selectedBox, opacityPopup, 드래그 상태(dragTpsl/dragScaleIn/dragSplitTp)
- **orderSlice**: 모든 주문 액션 (executeOrder/saveTpsl/scaleIn/addSplitTp/swapPosition/closePosition 등) + 일일 손실 한도 가드
...
```
- 포지션 종료 시: SCALE_IN은 수동 청산 시 즉시 취소, TP/SL 자연 체결 시 reconcile(15초) 내 취소
- 포지션 종료 시: SPLIT_TP는 Binance가 reduceOnly 주문 자동 취소 → UDS CANCELED 수신 시 제거
- store 파일은 load() 시 7일 초과 항목 자동 정리

### 일일 손실 한도
- **한도**: 당일 시작 시점 총자본(walletBalance − todayPnl)의 5%
- **리셋**: UTC 0시 기준 (Binance income API의 REALIZED_PNL 합산)
- **주문 차단**: `routes/dailyloss.js::checkDailyLoss()`를 `POST /api/order` 앞단에서 호출
- **UI 가드**: `orderSlice.openConfirm`이 `/api/daily-loss` 조회 후 remaining ≤ 0이면 확인 모달 차단
- **표시**: SidebarPanel 상단에 오늘 손실 / 한도 / 잔여분 표시 (useDailyLoss 1분 폴링)

### 포지션 스왑 (POST /api/swap)
- 현재 포지션 방향을 반대로 뒤집음 — 같은 수량 유지
- 절차: 기존 TP/SL + SCALE_IN + SPLIT_TP 전량 취소 → 시장가 청산 → 반대 방향 시장가 진입 → TP/SL ±2% 자동 등록
- 반대 진입 실패 시 closeOnly 플래그로 응답 → 프론트에서 criticalAlert 표시

### 스크린샷 자동 캡처 (useScreenshot)
- 포지션 open ↔ close 전이 시 `captureChart()`로 캔들/볼륨/RSI 캔버스 합성
- 진입·청산 지점에 SIDE 마커(롱=초록 ▲, 숏=빨강 ▼) 그림
- base64 PNG → `POST /api/screenshot` → `ScreenShot/YYYY-MM-DD_HHMMSS_SIDE_PRICE.png`로 저장
...
- 채널 타입: `channel_ep` (끝점), `channel_move` (몸통), `channel_offset` (미러선 폭 조절)
...
- 대상: RSI(period/OB/OS), FVG(lookback/mitigation), OB(swing/bos), Divergence(peak_lb/scan), SR(KDE 파라미터), EMA(배열)

### 알림 시스템
- **토스트 종류**: 일반(금색 테두리, 30초 자동닫힘) / sticky(빨강 테두리, 확인 버튼 필수)
- **포지션 종료 알림**: 롱/숏 포지션 종료 시 sticky 알림
- **추세선/채널/원 근접 알림**: 0.2% 이내 진입 → sticky, 0.3% 이상 이탈 시 해제 (히스테리시스)
- **RSI 알람** (`useAlertMonitor`): 5개 타임프레임(5m/15m/1h/4h/1d) 독립 WebSocket 감시
  - RSI 과매수(≥70) / 과매도(≤30) — 쿨다운(봉 길이 기준) + 히스테리시스(65/35 복귀 시 해제)
  - 다이버전스: 봉 마감 시 새로 발생한 것만 알림
  - 봉 마감: 타임프레임별 개별 설정
- **알림 설정** (`NotificationMenu`): 5TF × (RSI OB/OS, 다이버전스, 히든다이버전스, 봉마감) 체크박스

### 레버리지 변경
- 사이드바 레버리지 슬라이더 조작 → pendingLeverage 임시 저장 → 확인 버튼 클릭 시 적용
- 포지션 보유 중: `POST /api/leverage` 호출 후 스토어 업데이트 (포지션 레버리지 미만으로 감소 차단)
...
- **백엔드 → 프론트엔드**: pushService WebSocket (`ws://localhost:3001`)
- **프론트엔드**: useRealtimeData가 연결 관리, 메시지 타입별 refetch 트리거
- UDS 실패 시: 폴링(POLLING 상수) + reconcileWithBinance(15초)로 보완
...
- FVG/OB/SR/EMA: 캔버스 렌더 (`overlayRenderers.js` — 과거에 있던 SVG 컴포넌트 제거됨)
