...
│   │                             showLegPct(legPctRef/legVolRef+Up/Dn tspan) — 지그재그 레그 hover 라벨
│   │                               등락률 + 양봉/음봉 피크 거래량 + 직전 동일방향 레그의 같은 쪽 대비
...
│   │                             레그별로 봉범위/피크/직전레그/판정을 표로. 진행 중 레그도 포함
│   │                             화면이 쓰는 함수(legPeakVolume·findPrevSameDirLeg)를 그대로 호출
...
│   │                               { pct, i1, i2, prev } — prev = 직전 동일방향 레그(k-2)
│   │                             findPrevSameDirLeg — k-2가 없을 때 다른 구조에서 찾는 폴백 [LV7]
│   ├── legVolume.js           ← legPeakVolume()/fmtVol()/volChangePct() — 레그 **피크** 거래량 (순수 함수)
│   │                             { bars, up, dn } — 양봉/음봉 피크 분리 (각 { peak, idx } | null)
...
### 지그재그 레그 hover — 등락률 + 양봉/음봉 피크 거래량 (2026-08-12 / 피크 2026-08-13)
...
```
- **등락률**은 상승 초록 / 하락 빨강.
- **피크의 초록/빨강은 그 봉의 방향**을 뜻한다 (가격 방향 색과 의미가 일치).
  증감은 ↑↓ 화살표로만 보이고 **해석은 글자로 단정하지 않는다**
  (상승 레그인데 양봉 피크 ↓ = 동력 약화. 그 판단은 사용자 몫)
- 라벨에 "피크"라고 써 붙인다 — 숫자만 있으면 합계로 읽힌다
- **`[LV6]` 레그 방향에 해당하는 쪽만 표시** — 상승 레그는 ▲(양봉 피크), 하락 레그는 ▼(음봉 피크).
...
- **`[LV5]` 테이커(체결 주체) 기준 줄은 없다 — 2026-08-13 사용자 요청으로 제거.**
  한때 `피크`(캔들 색 기준) 아래에 `테이커`(매수/매도 체결량 기준) 줄을 나란히 두고
...
#### ⚠ 합계가 아니라 **피크**다 (`chart/legVolume.js` `[LV2]`, 사용자 확정)
...
- **`[LV7]` k-2가 없으면 다른 구조까지 뒤진다** (2026-08-13 사용자 지적으로 추가).
  사용자는 비교할 임펄스 레그를 **꼭짓점 2개짜리 구조로 하나씩 따로** 그린다
  (실제 저장 데이터 6개 중 4개가 레그 1개짜리) — 그런 구조는 두 칸 앞이 있을 수 없어
  비교가 통째로 안 떴다. 화면엔 비교할 상승 레그가 나란히 보이는데도.
  → `findPrevSameDirLeg`가 **모든 구조의 레그 중 같은 방향이면서 이 레그 시작 전에 끝난
  가장 최근 것**을 고른다. k-2가 있으면 그대로 쓴다(겹쳐 그린 구조에 끌려가지 않게).
  자동 ZZ 레그는 후보에서 제외 — 손으로 그린 것과 섞이면 무엇과 비교한 값인지 알 수 없다.
  시간 제한 없음(한 달 전 레그와 비교하는 게 이 사용법의 목적)
- 진행 중 레그(`liveSegment`)도 비교가 뜬다 — `Structures.jsx`가 소유 구조의
  `pts[n-3]→pts[n-2]`를 `prev`로 실어 보낸다 (`[R8]`). 꼭짓점이 3개 미만이면 `[LV7]` 폴백.
  **예전엔 `prev=null` 고정이라 진행 중 레그만 비교가 안 떴다** — 되돌리지 말 것
- 피크는 **`candlesRef.current`**로 계산 — React `candles`는 봉마감 때만 갱신돼
