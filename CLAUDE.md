...
│                                 API_BASE(localhost:3002), BN_PUBLIC, INTERVALS, RSI_H, RSI_GAP,
...
│   ├── usePosition.js         ← 포지션(long/short) + pending 주문 + scaleInOrders (5초 폴링)
│   ├── useTpsl.js             ← TP/SL + splitTps 조회 (폴링, hasPos일 때만) — long/short 각각
...
│       │                         헷지모드: 양쪽 포지션 모두 있을 때(hasBoth)만 플랜 버튼 잠금
│       ├── BalanceCard.jsx    ← 잔고 표시 카드
│       ├── MarketInfoCard.jsx ← 펀딩비(카운트다운 포함) + 공포·탐욕 지수
│       ├── PositionCard.jsx   ← 헷지모드: LONG/SHORT 각각 PositionCard 렌더 (추가진입·분할TP 아코디언 내장)
│       │                         PendingCard: 포지션 없을 때만 표시
...
│                                 hasLong/hasShort/hasPos/hasBoth/hasPending 상태 파생
├── main.jsx                   ← React 진입점
...
- **App.jsx 상태**: `hasLong`/`hasShort`/`hasPos`/`hasBoth` 파생 — `hasBoth`일 때 플랜 버튼/Swap 잠금
...
- **settingsSlice**: riskPct, leverage, interval_, indicators (localStorage 동기화)
...
- **orderSlice**: 모든 주문 액션 (executeOrder/saveTpsl/scaleIn/cancelScaleIn/moveScaleIn/addSplitTp/cancelSplitTp/moveSplitTp/closePosition/updatePendingTpsl/replacePendingOrder/deleteBox) + 일일 손실 한도 가드
...
- **표시**: SidebarPanel 상단에 오늘 손익 / 잔여 한도 표시 (useDailyLoss 1분 폴링)
