...
## ⚠ 쓰는 방식 — 애매한 표현과 주어 생략 금지

**사용자에게 하는 말, 코드 주석, 커밋 메시지, 이 문서 전부에 적용된다.**

- **주어를 반드시 쓴다.** "취소된다"가 아니라 "`placeTPSL`이 부분 손절을 취소한다"
- **비유·의인화를 쓰지 않는다.** "빨아들인다" "먹는다" "죽는다" 같은 말 대신
  "취소한다" "지운다" "멈춘다"처럼 실제 동작을 적는다
- **언제·무엇이·무엇을** 셋이 문장에 다 있는지 본다.
  "이럴 때 사라진다"는 안 된다 — 어떤 조건에서 무엇이 사라지는지 적는다
- **확인하지 않은 것을 단정하지 않는다.** 기존 주석에 적힌 예시도 옮기기 전에
  코드로 확인한다. 주석은 코드보다 먼저 낡는다
- **주석의 예시는 실제로 일어날 수 있는 상황이어야 한다.** 못 일어나는 예시는
  읽는 사람을 엉뚱한 곳으로 보낸다

⚠ 2026-09-04에 실제로 겪은 일: `binanceClient.js`의 주석이 "추가 진입 한 번이면
사라진다"라고 적고 있었는데 **그렇지 않았다.** 추가 진입 체결은 `placeTPSL`을
부르지 않는다. 그 주석을 확인 없이 사용자에게 옮겨 말했다가 되짚어서 고쳤다.
...
utils/
  ...
  slAlerts.js              손절이 모자랄 때 뜨는 빨간 줄의 문구 — **방향마다 한 줄, 두 가지뿐**
...
hooks/
  ...
  useCountdownPos.js       봉마감 카운트다운의 화면 위치 — 끌어서 옮기고 브라우저에 기억한다
                           (`countdownPos`, 차트 영역 좌상단 기준 px). 읽을 때마다 화면 안으로
                           되돌린다 — 창을 줄여 밖으로 나가면 **옮길 방법조차 없어진다**
...
- **최소 수량과 최소 금액은 별개다** — 거래소가 둘 다 본다. `calcPosition`이 둘 다 넘긴다.
  ...
  ※ `symbolInfo.js`의 SEED(못 받았을 때의 대비값)도 2026-09-06에 50으로 맞췄다.
    그전엔 `"100"`이라, 표를 못 받은 구간에서 작은 주문이 **두 배 수량**으로 올라갔다.
    ⚠ 이 값은 또 낡는다 — SEED에 기대는 구간을 늘리지 말 것
...
- 리플레이 시작 가능일은 심볼의 `onboardDate`다 (BTC 2019-09-08 / DOGE 2020-07-10) —
...
- **API 응답의 `orderId`는 언제나 문자열이다** (2026-09-02에 통일). 안 그러면 같은 필드가
  BTC에서는 숫자, ETH에서는 문자열이 되어 `===` 비교가 심볼에 따라 갈린다.
  새 응답에 주문번호를 담을 때도 `String(...)`으로 감쌀 것
...
- `reconcileWithBinance()` 60초 — 미체결·체결 정합, SL 없는 포지션 감시 → critical 알림
  - **뒷정리는 심볼별이다** (2026-09-02): 고아 사전 TP/SL · 남은 트리거 · 남은 추가 진입 ·
    TP/SL 재등록이 전부 **그 주문의 심볼**로 판정한다.
    ⚠ 조회에 실패한 심볼은 시야에서 빼고 **손대지 않는다** — 빈 값을 "포지션 없음"으로
      읽으면 통신이 한 번 튀는 것으로 멀쩡한 주문이 취소된다
  - 진입 시각 추적·무방비 경보는 아직 기본 심볼만 본다 (다른 심볼의 무방비는
    3초짜리 `watchAccount`가 잡는다)
...
- **자동 이어그리기** (`structAutoPivots.js`) — 커스텀 구조의 마지막 꼭짓점 뒤로 붙는 점선.
  ...
  - **사용자가 찍은 점 뒤는 자동 구조와 같은 알고리즘이어야 한다** (2026-09-09 사용자 확정).
    찍어둔 점이 자동 구조의 꼭짓점과 같은 자리면 **그 뒤 결과도 같아야 한다** —
    검산은 `tests/structAutoMirror.test.js`(자동 구조의 꼭짓점을 사용자 점으로 놓고 맞대어 본다)
  - ⚠ 그래서 **진행 중 봉도 훑는다.** 2026-08-26~09-09에는 그 봉을 빼고(`end = length - 2`)
    맨 끝에 "잠정 꼭짓점"을 하나 붙여 점선이 늘 현재 봉까지 닿게 했는데, 그 점은 피벗 판정도
    노이즈 필터도 거치지 않고 **직전 점의 반대 타입으로 고정**돼 붙었다. 자동 구조에는 없는
    점이라 이것 하나로 답이 갈렸다 — 실측 429가지 중 **387가지 불일치**(고친 뒤 0).
    되돌리지 말 것. 되돌리면 진행 중 봉이 같은 방향으로 새 극값을 만들 때 그 돌파가
    구조에 들어오지 않아 **CHoCH가 한 봉 늦는다**(화면·알림 둘 다. `useChochAlert`가 같은 함수를 부른다)
  - ※ **점선이 현재 봉에 안 닿을 수 있다 — 그게 맞다.** 자동 구조의 선이 끝나는 자리에서
    같이 끝난다 (실측 700회 중 현재 봉에 닿은 것이 361회). "항상 닿아야 한다"는 규칙을
    다시 만들지 말 것 — 그 규칙이 곧 위의 어긋남이다
  - ※ 남는 차이 하나: 진행 중 봉은 ATR이 커지며 노이즈 필터 문턱도 올라가, 봉 초반에 그려진
    꼭짓점이 같은 봉 후반에 사라질 수 있다. 자동 구조는 forward-only라 이 일이 없다.
...
- ⚠ **로그 눈금의 바닥은 `LOG_MIN`(1e-12) 하나다** — 네 곳(`padYDomain`·`getScales`·
  `shiftYDomain`·`zoomYDomain`)이 나눠 쓴다. 2026-09-06까지 그 값이 **`1`(=1달러)**이라,
  1달러 미만 코인은 로그를 켜면 **캔들이 화면 밖으로 사라졌다**
  (실측: DOGE 0.2 → 범위가 `0.99994~1.00106`, 최고가가 화면 500px 중 715,068px 자리).
  BTC·ETH는 바닥에 닿지 않아 값이 하나도 안 바뀐다. 상대값(최고가의 몇 분의 1)으로
  바꾸지 말 것 — 10만 배 움직인 코인의 아래쪽 정상 구간이 잘린다.
  검산: `tests/logScale.test.js`

### 세로 축 — 자동 맞춤과 `A`·`L` 버튼 (2026-09-06 사용자 요청)
가격 축 오른쪽 아래에 버튼 둘이 있다 (트레이딩뷰와 같은 자리). 캔버스가 아니라 **DOM 버튼**이다 —
축은 캔버스에 그려지지만 거기에는 클릭을 받을 방법이 없다.

| 버튼 | 무엇 |
|---|---|
| `A` | (`A`·`L`은 **시간 축의 오른쪽 끝**에 가로로 — 가격 축 칸으로 넘어가지 않는다) **화면 이동 모드**. 켜면 차트를 상하좌우로 끌 수 있다. 끄면 세로를 지금 보이는 봉에 다시 맞춰 **기존 동작으로 돌아온다** (**가로는 안 건드린다** — 그건 `resetDomain`이다) |
| `L` | **로그 눈금** (`isLog`, `chart_isLog`). 예전엔 상단바에 `Log`로 있었다 |

- ⚠ **`A`는 트레이딩뷰와 뜻이 반대다** (거기선 A 켜짐 = 자동 맞춤). 2026-09-06에 사용자가
  고른 방식이다 — "내가 켰을 때만 움직인다"가 규칙 하나로 끝나기 때문이다.
  끌다가 저절로 꺼지는 방식(트레이딩뷰)은 좌우로 끌 때의 **손떨림으로도 꺼져서**,
  왜 세로가 안 맞는지 화면에 드러나지 않는다. 되돌리지 말 것
- 왜 필요했나: 1시간봉에서 캔들 위/아래의 빈 영역을 볼 수 없어 4시간·일봉으로 바꿔야 했다
- 옮길 때 **세로 범위의 폭(로그면 배율)은 그대로**다 — 캔들의 가로세로 비율이 변하지 않는다
- 이동 모드에서는 **휠도 세로를 가로와 같은 배율로** 움직인다 (`zoomYDomain`, 커서 기준).
  다시 맞추면 옮겨 둔 화면이 휠 한 번에 돌아가고, 그렇다고 세로를 그대로 두면 가로만
  넓어져 **캔들이 세로로 길어 보인다** (2026-09-06 사용자 신고). 사진을 확대·축소하듯 움직인다
- 모드는 **저장하지 않는다** — 새로고침하면 늘 꺼진 상태(기존 동작)로 시작한다
- ⚠ **가격 축 위에서는 크로스헤어도 커서도 안 보인다 — 그게 맞다** (2026-09-06 사용자 확정).
  SVG 전체가 `cursor:none`인 것은 차트 안에서 크로스헤어를 직접 그리기 때문이다.
  축에 버튼을 놓았더니 "겨냥할 것이 안 보인다"가 되어 두 가지를 시도했다가 **둘 다 되돌렸다**:
  브라우저 십자 커서(우리가 그리는 것과 모양이 달라 커서가 두 개로 보였다)와,
  축까지 크로스헤어를 이어 그리기. **답은 버튼을 축 밖(시간 축 오른쪽 끝)으로 옮기는 것이었다** —
  캔들 영역 바로 아래라 크로스헤어를 보며 내려올 수 있다. 축에 특례를 다시 만들지 말 것
- TF·코인을 바꾸면 `applyInitialDomain`·`resetDomain`이 **모드를 끈다** —
  옛 가격대를 붙든 채 코인을 바꾸면(BTC 70,000 → DOGE 0.2) 캔들이 화면 밖이 된다
- 계산은 `scales.js`의 `shiftYDomain`(끌기) · `zoomYDomain`(휠) 둘 (순수 함수,
  `tests/shiftYDomain.test.js`).
  ⚠ **로그에서는 더하기가 아니라 곱하기다** — 선형 식을 그대로 쓰면 위로 갈수록 어긋난다
- ※ 트레이딩뷰의 `%`(퍼센트 눈금)는 만들지 않았다 (2026-09-06 사용자 판단) — 한 번에
  한 코인만 그리고, "몇 % 움직였나"는 측정 박스가 이미 답한다
- ※ 가격 축을 끌어 **세로 폭을 늘이는 것**은 아직 없다. 필요해지면 이 토대 위에 얹는다
...
| 어디 | 무엇 | 백업 |
|---|---|---|
| 브라우저 localStorage | 도형·플랜 박스(**심볼별**)·리스크/레버리지·지표 설정·알림·단축키·테마·연습 세션·**즐겨찾기 심볼**·패널 높이·카운트다운 위치 | ○ |
...
- **사람이 판단해야 하는 것** → 남는 게 맞다. 사용자가 눌러서 닫는다
  (일일 손실 체크 실패 · 분할 SL 재조정 실패)

### 손절이 모자랄 때의 빨간 줄 — **코인·방향마다 하나** (2026-09-06 통합)
띄우는 함수는 `orderWatcher.raiseNaked` 하나, 문구는 `utils/slAlerts.naked` 하나,
거두는 함수는 `orderWatcher.resolveNaked` 하나다. 라우트는 `raiseSlMissing`으로만 띄운다.

- 찾는 자리는 여섯이다: 진입 직후 SL 실패(`routes/order.js`) · 걸어둘 가격 없음 ·
  재시도 소진 · 60초 정합의 재등록 실패 · 3초 감시 · 60초 점검
- **문구는 상태만 말한다** (없다 / 일부만 덮는다 + 수량). 어느 경로로 실패했는지는
  화면이 아니라 로그가 답한다 — 사용자가 할 일은 여섯 다 "직접 걸어라" 하나다
- ⚠ 자리마다 문구를 만들지 말 것. 전에는 다섯 벌이었고, 손절이 정말 없으면
  **빨간 줄이 두 개** 떴다 (실패 문구 + 무방비 문구). 하나가 해소돼도 나머지가 남았다
- 3초 감시는 **15초(5회 관측)를 기다린다** — 체결 직후 SL이 걸리기까지의 틈에
  오경보를 내지 않기 위해서다. 6초째에 조용히 수리를 한 번 시도한다

### 손절을 **일부러 지웠으면** 빨간 줄이 뜨지 않는다 (2026-09-06 사용자 요청)
진입은 손절이 필수라(`middleware/validate`), 손절 없이 들고 가려면 차트에서 `×`로
지우는 것이 유일한 길이다. 그 선택에 계속 빨간 줄을 띄우면 사용자가 배너를 무시하게 된다.

- 표시(`slRemovedAt`)를 붙이는 곳은 `DELETE /api/tpsl` 하나 (지운 것이 **손절**일 때만)
- 표시가 막는 것 셋: 재시작 복구 · 60초 정합의 재등록 · **무방비 빨간 줄**
- ⚠ **거두는 세 곳이 이 기능의 절반이다.** 안 거두면 기록이 정리될 때까지(7일)
  그 방향이 조용해져서, 다음에 **진짜로** 손절이 빠져도 알리지 않는다:
  · `PUT /api/tpsl` — 손절을 다시 걸었다
  · `resolveNaked(…, "closed")` — 그 포지션이 닫혔다
  · `resolveNaked(…, "sl")` — 손절이 다시 보인다 (바이낸스 앱에서 걸어도 마찬가지)
- ※ 조용해지는 것은 **빨간 줄뿐이다.** 손절이 없다는 사실은 TP/SL 표시에 그대로 남는다
- ⚠ **우리가 낸 진입 기록이 있는 포지션에만 듣는다.** 표시는 그 기록에 붙기 때문에,
  바이낸스 앱에서 연 포지션의 손절을 지우면 15초 뒤 빨간 줄이 그대로 뜬다
  (그 포지션은 재시작 복구·60초 재등록의 대상도 아니라, 전부터 같은 경계였다)
- 검산: `tests/nakedAlert.test.js` (뜨는 조건 · 안 뜨는 조건 · 표시를 거두는 조건)
