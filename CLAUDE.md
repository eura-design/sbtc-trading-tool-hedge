...
│   │                             ⚠ PUT은 **TP를 걸기 전에 그 사이드 분할 TP를 전부 취소한다**
│   │                               (`cancelSplitTpsFor`, 2026-08-19). 단일 TP ↔ 분할 TP는
│   │                               양방향 배타다 — 자세한 규칙은 "주문 종류" 절
...
- ⚠ **단일 TP와 분할 TP는 배타적이다 — 나중에 건 쪽이 이긴다** (2026-08-19 사용자 확정).
  같이 걸리면 포지션의 200%를 팔려는 셈이다. **양방향 대칭이라 규칙이 하나다**
  - 분할 TP 등록 → 그 사이드 단일 TP 취소 (`POST /api/tpsl/split`)
  - 단일 TP 등록·수정 → 그 사이드 **분할 TP 전부** 취소 (`PUT /api/tpsl`의 `cancelSplitTpsFor`)
  - 두 방향 중 **하나만 남기지 말 것** — 한쪽이 뚫리면 공존 상태가 다시 생긴다
  - ⚠ 순서는 **등록 먼저, 취소 나중**(양쪽 다) — 뒤집지 말 것. 등록은 흔히 실패한다
    (롱 TP를 현재가 아래에 놓으면 바이낸스 `-2021`). **취소를 먼저 하면 그 실패 한 번에
    반대쪽이 통째로 날아간다** — 단일 TP를 걸다 실패하면 분할 TP 여러 개가, 분할 TP를
    걸다 실패하면 단일 TP가 사라지는데, 화면엔 "수정 실패"만 뜨고 왜 없어졌는지는 안 나온다
    - 대신 둘 다 살아 있는 창이 한 요청만큼 생긴다. 그 사이에 TP가 발동해도 포지션이 0이 되며
      바이낸스가 분할 TP(reduceOnly)를 자동 취소한다 — 결과가 의도한 것과 같아 실질 위험이 없다
    - 취소가 실패하면 공존이 남으므로 응답의 `splitFailed`로 올려 **에러 배너**를 띄운다.
      그때는 프론트가 로컬 `splitTps`를 비우지 않는다 — 거래소에 살아 있는 주문을 화면에서
      지우면 배너와 화면이 서로 다른 말을 한다 (`orderSlice.saveTpsl`의 `clearSplit`)
  - ⚠ SL은 대상이 아니다 (양방향 모두). 분할 TP는 익절이라 손절과 겹치지 않는다
  - ⚠ **페이퍼 브로커도 같은 규칙이다** (`paperBroker.setTpsl` / `addSplitTp`) — 한쪽만
    고치면 연습이 실거래와 다르게 체결된다. node로 양방향 대조 검산됨
  - ⚠ **진입 체결 시 자동 등록 경로는 규칙을 안 탄다** (`services/binanceClient.js placeTPSL` /
    페이퍼 `_setTpslFor`). 같은 사이드에 분할 TP를 걸어 둔 채로 **진입 주문을 또 내면**
    체결 시 단일 TP가 새로 걸려 공존한다 (실측 확인). 여기에 취소를 넣지 않은 이유는
    `orderWatcher` 재시도와 `recoveryService`가 같은 함수를 부르기 때문 —
    **서버 재시작 복구가 멀쩡한 분할 TP를 지울 수 있다**
    - **대신 입구를 막았다**: 포지션이 있는 사이드엔 플랜 박스를 못 그린다
      (위 "드래그 시스템" 절). 플랜 박스가 이 경로의 유일한 실용 입구였다
    - 남은 틈: 바이낸스 앱 등 **외부에서** 같은 사이드로 TP 붙은 진입을 내는 경우.
      이 앱 밖의 일이라 막을 방법이 없다
