...
│       │                           ※ 줄어든 과정: 제목 + `현재`/`변경` 두 줄 → `5x → 10x` 한 줄 → 버튼만
...
- ※ `기타/structure_zigzag.pine`(자동 ZZ의 트레이딩뷰 원본)도 **2026-08-22 제거**됐다.
  아래 ZZ 절에 "Pine도 같이 고쳐라"는 주의가 여러 번 나오는데, **더 이상 저장소에 없다** —
  트레이딩뷰 쪽 스크립트를 계속 쓴다면 거기서 직접 맞춰야 한다 (이 저장소가 알 수 없다)
...
- **Structure Zigzag (ZZ)**: `기타/structure_zigzag.pine` 포팅 (※ 그 .pine은 2026-08-22 제거됨) — 왼쪽 left_bars 봉만 보는 피벗(오른쪽 확인봉 없음),
  ※ **bias 규칙은 Custom Structure Zigzag(deriveStructure.js)와 동일하게 통일됨** (2026-08-12/13)
    ...
    `기타/structure_zigzag.pine`도 같이 수정했었다 (그 파일은 2026-08-22 제거 — 트레이딩뷰에서
    같은 스크립트를 쓴다면 **한쪽만 고치면 마크 개수가 어긋난다**).
  ...
  ※ **CHoCH 마크는 수동 구조(Structures.jsx의 ChochMarks)와 픽셀 단위로 같은 규칙** (2026-08-12 통일):
    ...
    - `기타/structure_zigzag.pine`도 같이 수정했었다 (교차점 + 점선→실선 전환 + 라벨 제거).
      ※ 그 파일은 2026-08-22 저장소에서 제거됐다
