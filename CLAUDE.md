...
│   │                             성격이 같은 버튼은 같은 크기여야 한다 — 치수를 컴포넌트에
│   │                             복사하지 말 것. 아래 어긋남이 전부 그렇게 생겼다
...
- ※ 포지션 없이 TP/SL만 미리 올리는 것도 안 된다: `closePosition:true`는 TIF가 `GTE_GTC`라
  `Time in Force (TIF) GTE can only be used with open positions`로 거절된다 (실측, trade_log)
...
| 신규 구조 기본 투명도 **0.25** (2026-08-24 — 그전엔 0.5) | 자동 ZZ와 초기 수치가 갈린다. 두 지표는 팝업 구성이 같아서 숫자만 다르면 다른 것처럼 보인다 |
...
- ※ `zz`의 **show_choch·max_choch는 두 곳에 있다** (2026-08-14 사용자 요청):
  ...
  - `zz.show_legvol`·`alert_choch`·`opacity`는 **팝업에만** 있다 (도형 공통 UI라 메뉴에 자리가 없다)
    ...
    - `opacity` 기본값은 **0.25**다 (2026-08-24) — 수동 구조 신규 기본값과 같은 값.
